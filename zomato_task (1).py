...
Original file is located at
    https://colab.research.google.com/drive/153cg1ZHimtg4h98ornFRGYmnO_QEq7ke
"""
...
Zomato.to_csv("new_zomato.csv", index=False)
...
pie_country_wise = px.pie(country_wise_data, values=country_wise_data.values, names=country_wise_data.index, color_discrete_sequence=px.colors.sequential.Rainbow)
...
pie_country_wise.update_layout(title_text="Zomato's around the World", title_x=0.5)
...
pie_rating_wise = px.pie(rating_wise_city_df, values=rating_wise_city_df.values, names=rating_wise_city_df.index, color_discrete_sequence=px.colors.sequential.Rainbow, hole=0.6)
...
country_wise_df = Zomato_data[Zomato_data['Country']=='India']
city_count = (country_wise_df['City'].value_counts())
fig = px.bar(city_count, x=city_count.index, y=city_count.values,labels={'x': "Cities","y": "Number of Restaurants"},color_discrete_sequence=px.colors.qualitative.G10)
...
fig = px.choropleth(cmap_df, locations=cmap_df.index, locationmode='country names',color=cmap_df.values ,color_continuous_scale=px.colors.sequential.Reds_r)
...
app.layout = html.Div(children=[
    ...
    html.Div(children=[
            ...
            html.Div(children='dashboard which gets to know about Zomato')],
...
#This is the third row of the dashboard
   ...
   #This is the fourth row of the dashboard
    ...
    html.Div(children=[
            ...
            html.Div(children='Created by: Gajalakshmi')],
            style={'textAlign': 'center','backgroundColor':'#E23744','color': 'white','font-family':['Open Sans','sans-serif'], 
...
def city_count(cities_dropdown):
...
@app.callback(
    ...
    [Input("countries_dropdown", "value")],suppress_callback_exceptions=True)
def update_city(countries_dropdown):
    df_result = Zomato_data[Zomato_data['Country']==countries_dropdown]
